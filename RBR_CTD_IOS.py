...
# EXPORT_FILES(dest_dir = 'C:/Projects/RBR_CTD_Pycharm/2019-107-example/',
#              file = '066024_20190823_0915_CTD_Data.rsk', year = '2019', cruise_number='107')
...
# EXPORT_FILES(dest_dir = 'C:/Projects/RBR_CTD_Pycharm/2021-037/',
#              file = '204848_20210203_2358.rsk', year = '2021', cruise_number='037',
#              event_start=3)

# -------------------  Exploring if we can input mulitple .rsk files----------------------

def EXPORT_MULTIFILES(dest_dir, all_last,  # year, cruise_number,
                      event_start):  # ALL or LAST, depends on event log, see which profiles to extract
    """
    ...
    for k in range(0, n_files, 1):
        ...
        if all_last == 'ALL':  # get all profiles within each .rsk file
            ...
            for i in range(0, n_profiles, 1):
                ...
                # downcast_name = filename.split("/")[-1][0:-4].upper() + "_profile" + str(current_profile).zfill(
                #     4) + "_DOWNCAST.csv"  # downcast file name
                # upcast_name = filename.split("/")[-1][0:-4].upper() + "_profile" + str(current_profile).zfill(
                #     4) + "_UPCAST.csv"  # upcast file name
                profile_name = filename.split("/")[-1][0:-4].upper() + "_profile" + str(current_profile).zfill(
                    4) + ".csv"  # profile name
                ...
                profile_data = pd.concat(
                    [downcast_dataframe, upcast_dataframe])  # combine downcast and upcast into one profile
                # profile_data['Event'] = e_num + 1

                # combine profiles into one file # not needed for multifile
                # ctd_data = ctd_data.append(profile_data, ignore_index=True)
...
# EXPORT_MULTIFILES(dest_dir = 'C:/Projects/RBR_CTD_Pycharm/2021-039_Python_Run/',
#                   year = '2021', cruise_number='039', all_last='LAST', event_start=1)
...
def READ_EXCELrsk(dest_dir, year, cruise_number, event_start, all_last):
   ...
   """
    ...
    for k in range(0, n_files, 1):
        ...
        # hourstonh change the range depending on if "All" or "Last"
        ...
        for i in range(range_start, n_times):
            ...
            # hourstonh fix split character to account for forward and double-backward slashes
            ...
            profile_data.to_csv(dest_dir + profile_name)  # export each profile #, index=False

            # output_filename = year + "-" + cruise_number + "_CTD_DATA.csv"
            # all data file name#ctd_data.to_csv(dest_dir + output_filename)

        if all_last == 'ALL':  # hourstonh
            event_start = current_profile
        # ctd_data.to_csv(dest_dir + output_filename)


# READ_EXCELrsk(dest_dir='C:/Projects/RBR_CTD_Pycharm/2021-038/', year='2021', cruise_number='038',
#               event_start = 341, all_last = 'ALL')

# ------------ step 1b: .rsk file in full structure - combine profiles into one .csv file ------------

# MERGE_FILES(dest_dir = '/home/guanl/Desktop/Projects/RBR/Processing/2020-085/CTD data/', year = '2020',
#             cruise_number= '085')


def MERGE_FILES(dest_dir, year, cruise_number, event_from='filename'):
    """
    Read in multiple csv file and output one csv format
    Inputs:
        - folder, year, cruise_number
        - hourstonh: event_from: where to get the event numbers. "filename" or "header-merge"
    Outputs:
        - csv file: csv files containing the profile data
   """
    files = os.listdir(dest_dir)  # list all the files in dest_dir
...
def CREATE_META_DICT(dest_dir, file, year, cruise_number):
    ...
    meta_dict['number_of_profiles'] = meta_csv['Value'][meta_csv['Name'] == 'number_of_profiles'].values[
        0]  # CHOOSE !!! if multiple rsk
...
# metadata = CREATE_META_DICT(dest_dir = 'C:/Projects/RBR_CTD_Pycharm/2021-070/',
#                             file = '204694_20211018_1808.rsk', year = '2021',
#                             cruise_number= '070')
...
#     index = ['Conductivity', 'Temperature', 'Pressure_Air', 'Fluorescence', 'Oxygen:Dissolved:Saturation', 'Pressure',
#              'Depth', 'Salinity:CTD', ' ', ' ', 'Cast_direction', 'Event_number', 'Date', 'TIME:UTC']
#     unit = ['mS/cm', 'deg C (ITS90)', 'decibar', 'mg/m^3', '%', 'decibar', 'Metres', 'PSS-78', ' ', ' ', 'n/a', 'n/a',
#             'n/a', 'n/a']
#     input_format = ['R4', 'R4', 'R4', 'R4', 'R4', 'R4', 'R4', 'R4', ' ', ' ', ' ', 'I4', 'D:dd/mm/YYYY', 'T:HH:MM:SS']
#     output_format = ['R4:F11.4', 'R4:F9.4', 'R4:F7.1', 'R4:F8.3', 'R4:F11.4', 'R4:F7.1', 'R4:F7.1', 'R4:F9.4', ' ',
#     ' ', ' ', 'I:I4', 'D:YYYY/mm/dd', 'T:HH:MM:SS']
...
def ADD_6LINEHEADER_2(dest_dir, year, cruise_number):
    ...
    drop_list = ['Temperature .1(Â°C)', 'speedofsound(m/s)', 'Temperature .1', 'Temperature.1',
                 'Speed of sound ', 'Speed of sound', 'specificconductivity(ÂµS/cm)',
    ...
    index_list = ['Conductivity', 'Temperature', 'Pressure_Air', 'Fluorescence', 'Oxygen:Dissolved:Saturation',
                  'Pressure', 'Depth', 'Salinity:CTD', 'Cast_direction', 'Event_number', 'Date', 'TIME:UTC']
    unit_list = ['mS/cm', 'deg C(ITS90)', 'decibar', 'mg/m^3', '%', 'decibar', 'meters', 'PSS-78', 'n/a', 'n/a', 'n/a',
                 'n/a']
    input_format_list = ['R4', 'R4', 'R4', 'R4', 'R4', 'R4', 'R4', 'R4', ' ', 'I4', 'D:dd/mm/YYYY', 'T:HH:MM:SS']
    output_format_list = ['R4:F11.4', 'R4:F9.4', 'R4:F7.1', 'R4:F8.3', 'R4:F11.4', 'R4:F7.1', 'R4:F7.1', 'R4:F9.4', ' ',
                          'I:I4', 'D:YYYY/mm/dd', 'T:HH:MM:SS']
    ...
    # hourstonh: if plan shapes not aligned, then may need to add another variable to drop_list
    ctd_data_header = header.append(ctd_data)
    ctd_data_header.to_csv(dest_dir + output_name, index=False, header=False)


# ADD_6LINEHEADER_2(dest_dir = 'C:/Projects/RBR_CTD_Pycharm/2021-038/', year = '2021',
#                   cruise_number = '038')
...
# Plot_Track_Location(dest_dir='C:/Projects/RBR_CTD_Pycharm/2020-085/', year = '2020',
#                     cruise_number= '085',
#                     left_lon=-131.5, right_lon=-131.8, bot_lat=52.5, top_lat=52.7)
...
def Plot_Track_Location(dest_dir, year, cruise_number, left_lon, right_lon, bot_lat, top_lat):
    ...
    parallels = np.arange(bot_lat, top_lat,
                          0.5)  # parallels = np.arange(48., 54, 0.2), parallels = np.linspace(bot_lat, top_lat, 10)
    ...
    x, y = (lon, lat)
    map.coastlines()
    gl = map.gridlines(crs=ccrs.PlateCarree(), linewidth=0.5, color='black', alpha=0.5, linestyle='--',
                       draw_labels=True)
    gl.top_labels = False
    gl.left_labels = True
...
# Plot_Track_Location(dest_dir='C:/Projects/RBR_CTD_Pycharm/2021-038/', year = '2021', cruise_number= '038',
#                     left_lon=-115, right_lon=-126, bot_lat=67, top_lat=73)
...
# PLOT_PRESSURE_DIFF(dest_dir = '/home/guanl/Desktop/Projects/RBR/Processing/2020-085/CTD data/', year= '2020',
#                    cruise_number= '085')
...
# PLOT_PRESSURE_DIFF(dest_dir = 'C:/Projects/RBR_CTD_Pycharm/2021-038/', year= '2021', cruise_number= '038',
...
# cast, cast_d, cast_u = CREATE_CAST_VARIABLES(year = '2021' , cruise_number = '045',
#                                              dest_dir = 'C:/Projects/RBR_CTD_Pycharm/2021-045/IOS_SHELL_STEPS',
#                                              input_ext = '_CTD_DATA-6linehdr.csv')
...
def First_Plots(year, cruise_number, dest_dir, input_ext):
    ...
    for i in range(0, len(cast), 1):
        ax.plot(cast_d['cast' + str(i + 1)].Salinity, cast_d['cast' + str(i + 1)].Pressure, color=color[i],
                label='cast' + str(i + 1))
    ...
    fig, ax = plt.subplots()
    for i in range(0, len(cast), 1):
        ax.plot(cast_d['cast' + str(i + 1)].Temperature, cast_d['cast' + str(i + 1)].Pressure, color=color[i],
                label='cast' + str(i + 1))
        # ax.plot(cast_u['cast' + str(i+1)].Temperature, cast_u['cast' + str(i+1)].Pressure, '--', color=color[i],
        #         label= 'cast' + str(i+1))
    ...
    fig, ax = plt.subplots()
    for i in range(0, len(cast), 1):
        ax.plot(cast_d['cast' + str(i + 1)].Conductivity, cast_d['cast' + str(i + 1)].Pressure, color=color[i],
                label='cast' + str(i + 1))
        # ax.plot(cast_u['cast' + str(i+1)].COnductivity, cast_u['cast' + str(i+1)].Pressure, '--', color=color[i],
        #         label= 'cast' + str(i+1))
    ...
    for var in vars:
        if var == 'Oxygen':
            ...
            fig, ax = plt.subplots()
            for i in range(0, len(cast), 1):
                ax.plot(cast_d['cast' + str(i + 1)].Oxygen, cast_d['cast' + str(i + 1)].Pressure, color=color[i],
                        label='cast' + str(i + 1))
            # ax.plot(cast_u['cast' + str(i+1)].Oxygen, cast_u['cast' + str(i+1)].Pressure, '--', color=color[i],
            #         label= 'cast' + str(i+1))
    ...
    for var in vars:
        if var == 'Fluorescence':
            fig, ax = plt.subplots()
            for i in range(0, len(cast), 1):
                ax.plot(cast_d['cast' + str(i + 1)].Fluorescence, cast_d['cast' + str(i + 1)].Pressure, color=color[i],
                        label='cast' + str(i + 1))
            # ax.plot(cast_u['cast' + str(i+1)].Fluorescence, cast_u['cast' + str(i+1)].Pressure, '--',
    ...
    fig, ax = plt.subplots()
    for i in range(0, len(cast), 1):
        ax.plot(cast_d['cast' + str(i + 1)].Salinity, cast_d['cast' + str(i + 1)].Temperature, color=color[i],
                label='cast' + str(i + 1))
    ...
    for i in range(0, len(cast), 1):
        ax.plot(cast_d['cast' + str(i + 1)].Conductivity[0:20], cast_d['cast' + str(i + 1)].Depth[0:20], color=color[i],
                label='cast' + str(i + 1))
        ax.plot(cast_u['cast' + str(i + 1)].Conductivity[-20:-1], cast_u['cast' + str(i + 1)].Depth[-20:-1], '--',
                color=color[i],
                label='cast' + str(i + 1))
...
def CALIB(var, var_downcast, var_upcast, correction_value, metadata_dict, zoh):
    """
     ...
     Inputs:
         ...
         - zoh: if "no", zero-order hold correction was not applied, else "yes"
     Outputs:
...
def FILTER(var_downcast, var_upcast, window_width, sample_rate, time_constant, filter_type,
           metadata_dict):
    """
    ...
    for i in range(1, cast_number + 1, 1):
        ...
        #       b, a, var2['cast' + str(i)].Fluorescence)      #!!!!!  might need!
...
#   cast_d_clip, cast_u_clip, window_width = 6, sample_rate = 8, time_constant = 1/8,
#   filter_type = 1, metadata_dict = metadata) # n = 5 should be good.


# plot to check values before and after filtering
def plot_filter(cast_d_filtered, cast_d_clip, dest_dir):
    ...
    for var in vars:
        if var == 'Oxygen':
            ...
            fig, ax = plt.subplots()
            ax.plot(cast_d_clip['cast1'].Fluorescence, cast_d_clip['cast1'].Pressure, color='blue',
                    label='Pre-filtering')
...
def plot_shift_c(cast_d_shift_c, cast_d_filtered, dest_dir):
    """
    inputs:
        - cast_d_shift_c
        - cast_d_filtered
    """
    fig, ax = plt.subplots()
    ax.plot(cast_d_filtered['cast1'].Salinity, cast_d_filtered['cast1'].Pressure,
            color='blue', label='Pre-shift')
    # ax.plot(cast_u_filtered['cast1'].Salinity, cast_u_filtered['cast1'].Pressure,
    #         '--', color='blue', label='Pre-shift')
    ax.plot(cast_d_shift_c['cast1'].Salinity, cast_d_shift_c['cast1'].Pressure,
    ...
    # TS Plot
    fig, ax = plt.subplots()
    ax.plot(cast_d_filtered['cast1'].Salinity, cast_d_filtered['cast1'].Temperature,
            color='blue', label='Pre-shift')
    # ax.plot(cast_u_filtered['cast1'].Salinity, cast_u_filtered['cast1'].Temperature,
    #         '--', color='blue',
    # label='Pre-shift')
    ax.plot(cast_d_shift_c['cast1'].Salinity, cast_d_shift_c['cast1'].Temperature,
            color='red', label='Post-shift')
...
#   cast_d_shift_c, cast_u_shift_c, shifted_scan_number = -11, metadata_dict = metadata)
# advance oxygen data by 11 scans
# except KeyError:
...
# ----------------------------- Add Fluorescence shift ---------------------------------------

def SHIFT_FLUORESCENCE():
    # TODO confirm this step with Jessy
    return
...
def DERIVE_OXYGEN_ML_L():
    # TODO confirm which equations to use from Jessy
    return
...
#   cast_d_shift_o, cast_u_shift_o, metadata_dict = metadata)

# Plot Salinity and T-S to check the index after shift
# Salinity

def plot_processed(cast_d_wakeeffect, cast_d_shift_o, cast_d, cast, dest_dir):
    ...
    #         label='Before_processing')
    ax.plot(cast_d_wakeeffect['cast1'].Salinity, cast_d_wakeeffect['cast1'].Pressure,
            color='red', label='After_processing')
    # ax.plot(cast_u_wakeeffect['cast1'].Salinity, cast_u_wakeeffect['cast1'].Pressure,
    #         '--', color='red', label='cast1')
    ax.invert_yaxis()
    ax.xaxis.set_label_position('top')
    ax.xaxis.set_ticks_position('top')
    ax.set_xlabel('Salinity')
    ...
    fig, ax = plt.subplots()
    # for i in range (0, len(cast_d), 1):
    #    #ax.plot(cast_d['cast' + str(i+1)].Temperature, cast_d['cast' + str(i+1)].Pressure,
    #    color=color[i], label= 'cast' + str(i+1))
    #    ax.plot(cast_d_wakeeffect['cast' + str(i+1)].Temperature,
    #    cast_d_wakeeffect['cast' + str(i+1)].Pressure, '--', color=color[i],
    #    label='cast' + str(i+1))
    ax.plot(cast_d['cast1'].Temperature, cast_d['cast1'].Pressure, color='blue',
            label='Pre_Processing')
    # ax.plot(cast_u['cast1'].Temperature, cast_u['cast1'].Pressure, '--', color='blue',
    #         label='cast1')
    ax.plot(cast_d_wakeeffect['cast1'].Temperature, cast_d_wakeeffect['cast1'].Pressure,
            color='red', label='Post_Processing')
    # ax.plot(cast_u_wakeeffect['cast1'].Temperature, cast_u_wakeeffect['cast1'].Pressure,
    #         '--', color='red', label='cast1')
    ax.invert_yaxis()
    ax.xaxis.set_label_position('top')
    ax.xaxis.set_ticks_position('top')
    ...
    fig, ax = plt.subplots()
    # for i in range(0, len(cast_d), 1):
    # ax.plot(cast_d['cast' + str(i+1)].Conductivity, cast_d['cast' + str(i+1)].Pressure,
    #         color=color[i], label= 'cast' + str(i+1))
    # ax.plot(cast_d_wakeeffect['cast' + str(i + 1)].Conductivity,
    #         cast_d_wakeeffect['cast' + str(i + 1)].Pressure,
    # color=color[i], label='cast' + str(i + 1))
    ax.plot(cast_d['cast1'].Conductivity, cast_d['cast1'].Pressure, color='blue',
            label='Pre_Processing')
    # ax.plot(cast_u['cast2'].Conductivity[1:], cast_u['cast2'].Pressure[1:], '--',
    #         color='red', label='cast2')
    ax.plot(cast_d_wakeeffect['cast1'].Conductivity, cast_d_wakeeffect['cast1'].Pressure,
            color='red', label='Post_Processing')
    # ax.plot(cast_u_wakeeffect['cast1'].Conductivity, cast_u_wakeeffect['cast1'].Pressure,
    #         color='red', label='cast1')
    ax.xaxis.set_label_position('top')
    ax.xaxis.set_ticks_position('top')
    ax.set_xlabel('Conductivity (S/cm)')
    ...
    for var in vars:
        if var == 'Oxygen':
            ...
            fig, ax = plt.subplots()
            # for i in range(0, len(cast_d), 1):
            # ax.plot(cast_d['cast' + str(i+1)].Oxygen, cast_d['cast' + str(i+1)].Pressure,
            #         color=color[i], label= 'cast' + str(i+1))
            # ax.plot(cast_d_wakeeffect['cast' + str(i + 1)].Oxygen,
            #         cast_d_wakeeffect['cast' + str(i + 1)].Pressure,
            #         color=color[i], label='cast' + str(i + 1))
            ax.plot(cast_d['cast1'].Oxygen, cast_d['cast1'].Pressure, color='blue',
                    label='Pre_Processing')
            # ax.plot(cast_u['cast1'].Oxygen, cast_u['cast1'].Pressure, '--', color='blue',
            #         label='cast1')
            ax.plot(cast_d_wakeeffect['cast1'].Oxygen, cast_d_wakeeffect['cast1'].Pressure,
                    color='red', label='Post_Processing')
            # ax.plot(cast_d_wakeeffect['cast1'].Oxygen, cast_d_wakeeffect['cast1'].Pressure,
            #         color='red', label='cast1')
            ax.invert_yaxis()
            ...
            fig, ax = plt.subplots()
            # for i in range(0, len(cast_d), 1):
            # ax.plot(cast_d['cast' + str(i+1)].Fluorescence,
            # cast_d['cast' + str(i+1)].Pressure, color=color[i], label= 'cast' + str(i+1))
            # ax.plot(cast_d_wakeeffect['cast' + str(i + 1)].Fluorescence,
            # cast_d_wakeeffect['cast' + str(i + 1)].Pressure, '--', color=color[i],
            # label='cast' + str(i + 1))
            ax.plot(cast_d['cast1'].Fluorescence, cast_d['cast1'].Pressure, color='blue',
                    label='Pre Processing')
            # ax.plot(cast_u['cast1'].Fluorescence, cast_u['cast1'].Pressure, '--', color='blue',
            #         label='cast1')
            ax.plot(cast_d_wakeeffect['cast1'].Fluorescence, cast_d_wakeeffect['cast1'].Pressure,
                    color='red', label='Post Processing')
            # ax.plot(cast_u_wakeeffect['cast1'].Fluorescence,
            #         cast_u_wakeeffect['cast1'].Pressure, color='red', label='cast1')
            ax.invert_yaxis()
            ax.xaxis.set_label_position('top')
            ax.xaxis.set_ticks_position('top')
            ax.set_xlabel('Fluorescence (ug/L)')  # Check unit here
...
def write_file(cast_number, cast_original, cast_final, oxy, metadata_dict):
    ...
    if oxy == 'yes':
        print('{:>8}'.format('5') + " " + '{:33}'.format(
            list(cast_final['cast' + str(cast_number)].columns)[4]) +
              ...
              '{:15}'.format(str(np.nanmin(
                  cast_final['cast' + str(cast_number)]['Fluorescence:URU'].astype(np.float)))) +
        ...
        print('{:>8}'.format('6') + " " + '{:33}'.format(
            list(cast_final['cast' + str(cast_number)].columns)[5]) + '{:15}'.format(
            "n/a") + '{:15}'.format(str(np.nanmin(
                cast_final['cast' + str(cast_number)]['Number_of_bin_records'].astype(np.float)))) +
              '{:14}'.format(
            str(np.nanmax(cast_final['cast' + str(cast_number)]['Number_of_bin_records'].astype(np.float)))))
...
# def write_history(cast_original, cast_clip, cast_filtered, cast_shift_c,
#                   cast_shift_o, cast_wakeeffect, cast_binned, cast_final,
#                   metadata_dict, cast_number):
def write_history(cast_original, cast_clip, cast_filtered, cast_shift_c, cast_shift_o,
                  cast_wakeeffect, cast_binned, cast_final, cast_number):
    """
    inputs:
        - cast_original:
        - cast_clip:
...
def main_header(dest_dir, n_cast, meta_data, cast, cast_d, cast_d_clip, cast_d_filtered,
                cast_d_shift_c, cast_d_shift_o, cast_d_wakeeffect, cast_d_binned,
                cast_d_final, oxy):
    """
    inputs:
        - dest_dir
    ...
    try:
        ...
        write_history(cast_d, cast_d_clip, cast_d_filtered,
                      ...
                      cast_d_binned, cast_d_final, cast_number=n_cast)  # metadata_dict=meta_data,
...
# meta_data = metadata, cast_d = cast_d, cast_d_clip = cast_d_clip,
# cast_d_filtered = cast_d_filtered,
# cast_d_shift_c = cast_d_shift_c, cast_d_shift_o = cast_d_shift_o,
# cast_d_wakeeffect = cast_d_wakeeffect, cast_d_binned = cast_d_binned,
# cast_d_final = cast_d_final)
...
# DON"T FORGET TO CHANGE THE SHEETNAME TO Profile_annotation or openpyxl won't read it.
working_dir = 'C:/Projects/RBR_CTD_Pycharm/2021-080/'
data_year = '2021'
data_cruise_number = '080'
...
def first_step(dest_dir, year, cruise_number, input_ext, rsk_file, event_start, all_last,
               left_lon, right_lon, bot_lat, top_lat, file_option):  # file_options: single, multi, rsk_excel
    ...
    if file_option == 'single':
        ...
        EXPORT_MULTIFILES(dest_dir, all_last, event_start)  # year, cruise_number,
...
# input_ext '_CTD_DATA-6linehdr.csv' for first steps, all_last 'ALL' or 'LAST',
# event_start comes from cast list or cruise log
# file_option 'single', 'multi', or 'from_excel' !!! if 'from_excel enter the file in the function

# first_step(dest_dir=dest_dir, year=year, cruise_number=cruise_number,
#            input_ext='_CTD_DATA-6linehdr.csv', rsk_file='204694_20211216_1628.rsk',
#            event_start=1,
# all_last='ALL', left_lon=-129, right_lon=-127, bot_lat=49.5, top_lat=51.5,
# file_option='from_excel')


metadata = CREATE_META_DICT(dest_dir=working_dir, file='204694_20211216_1628.rsk', year=data_year,
                            cruise_number=data_cruise_number)
...
def second_step(dest_dir, year, cruise_number, metadata_dict, zoh, correction_value, oxy,
    ...
    if zoh == 'yes':
        ...
        PLOT_PRESSURE_DIFF(dest_dir, year, cruise_number, input_ext='_CTD_DATA-6linehdr_corr_hold.csv')
    ...
    cast_d_filtered, cast_u_filtered = FILTER(cast_d_clip, cast_u_clip, window_width=6,
                                              sample_rate=8,
                                              time_constant=1 / 8, filter_type=1,
                                              metadata_dict=metadata)  # n = 5 should be good.

    plot_filter(cast_d_filtered, cast_d_clip, dest_dir)

    cast_d_shift_c, cast_u_shift_c = SHIFT_CONDUCTIVITY(cast_d_filtered, cast_u_filtered,
                                                        shifted_scan_number=2,
                                                        metadata_dict=metadata)  # delay conductivity data by 2 scans

    plot_shift_c(cast_d_shift_c, cast_d_filtered, dest_dir)

    if oxy == 'yes':
        cast_d_shift_o, cast_u_shift_o = SHIFT_OXYGEN(cast_d_shift_c, cast_u_shift_c,
                                                      shifted_scan_number=-11,
    ...
    plot_processed(cast_d_wakeeffect, cast_d_shift_o, cast_d, cast, dest_dir)

    cast_d_binned, cast_u_binned = BINAVE(cast_d_wakeeffect, cast_u_wakeeffect, interval=1,
                                          metadata_dict=metadata)
...
second_step(working_dir, data_year, data_cruise_number, metadata_dict=metadata, zoh='yes',
            correction_value=0, oxy='yes',
